...
public class RCC_FixedCamera : RCC_Singleton<RCC_FixedCamera> {
    ...
    public void ChangePosition() {
        ...
        if (Physics.Raycast(target.position + Vector3.up * 3f, Quaternion.AngleAxis(randomizedAngle, target.up) * target.forward, out hit, maxDistance) && !hit.transform.IsChildOf(target) && !hit.collider.isTrigger) {
            ...
            string log = $"use -> Fixed Camera ChangePosition: {transform.position.ToString("F1")}\n";
            transform.position = hit.point + Vector3.up * Random.Range(.5f, 2.5f);
            transform.LookAt(target.position);
            transform.position += transform.forward * 5f;
            log += $"end use -> Fixed Camera ChangePosition: {transform.position.ToString("F1")}";
            Debug.Log($"{log}");

        } else {

            string log = $"use -> Fixed Camera ChangePosition2: {transform.position.ToString("F1")}\n";
            ...
            transform.LookAt(target.position);
            transform.position += transform.forward * 5f;
            log += $"end use -> Fixed Camera ChangePosition2: {transform.position.ToString("F1")}";
            Debug.Log($"{log}");
