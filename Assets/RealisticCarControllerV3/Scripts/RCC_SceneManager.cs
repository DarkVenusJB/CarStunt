...
public class RCC_SceneManager : RCC_Singleton<RCC_SceneManager>
{
    ...
    public bool
        registerLastSpawnedVehicleAsPlayerVehicle = true; //  Registers the lastly spawned vehicle as player vehicle.
    ...
    private void Awake()
    {
        ...
        if (RCC_Settings.Instance.overrideFixedTimeStep) Time.fixedDeltaTime = RCC_Settings.Instance.fixedTimeStep;

        // Overriding FPS.
        if (RCC_Settings.Instance.overrideFPS) Application.targetFrameRate = RCC_Settings.Instance.maxFPS;
...
#region ONDESTROYED
    ...
    public IEnumerator GetAllTerrains()
    {
        ...
        if (allTerrains != null && allTerrains.Length >= 1)
        {
            ...
            for (int i = 0; i < allTerrains.Length; i++)
            {
                ...
                {
                    Debug.LogError("Terrain data of the " + allTerrains[i].transform.name +
                                   " is missing! Check the terrain data...");
                    yield return null;
                }
            }
            ...
            for (int i = 0; i < terrains.Length; i++)
            {
                ...
                terrains[i].mSplatmapData = allTerrains[i].terrainData
                    .GetAlphamaps(0, 0, terrains[i].alphamapWidth, terrains[i].alphamapHeight);
                terrains[i].mNumTextures = terrains[i].mSplatmapData.Length /
                                           (terrains[i].alphamapWidth * terrains[i].alphamapHeight);
            }
    ...
    public void RegisterPlayer(RCC_CarControllerV3 playerVehicle)
    {
        ...
        if (activePlayerCamera)
            activePlayerCamera.SetTarget(CarController.Instance.Player.transform, activePlayerVehicle, true);

        if (loadCustomizationAtFirst)
            RCC_Customization.LoadStats(activePlayerVehicle);
    ...
    public void RegisterPlayer(RCC_CarControllerV3 playerVehicle, bool isControllable)
    {
        ...
        if (activePlayerCamera)
            activePlayerCamera.SetTarget(CarController.Instance.Player.transform, activePlayerVehicle);

        if (loadCustomizationAtFirst)
            RCC_Customization.LoadStats(Instance.activePlayerVehicle);
    ...
    {
        if (!activePlayerVehicle || !activePlayerVehicle.canControl ||
            !activePlayerVehicle.gameObject.activeInHierarchy || !activePlayerVehicle.enabled)
        {
